...
async def start_complaint(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        utils.get_text('choose_faculty', context),
...
async def handle_faculty_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 🔙 Orqaga tugmasi
    if faculty_text == utils.get_text('btn_back', context):
        from main import start
        return await start(update, context)

    faculty_code = utils.get_faculty_code(faculty_text, context)

    if not faculty_code:
        await update.message.reply_text(
            f"⚠️ {utils.get_text('choose_faculty', context)}",
            reply_markup=get_faculties_keyboard(context)
        )
        return
    ...
    await update.message.reply_text(
        f"📍 {utils.get_text('choose_direction', context)}",
...
async def handle_direction_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if faculty == 'magistratura':
        ...
        await update.message.reply_text(
            f"📍 {utils.get_text('choose_edu_type', context)}",
...
async def handle_education_type_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 🔙 Orqaga tugmasi
    if edu_type_text == utils.get_text('btn_back', context):
    ...
    edu_type_code = utils.get_education_type_code(edu_type_text, context)

    if not edu_type_code:
        await update.message.reply_text(
            f"⚠️ {utils.get_text('choose_edu_type', context)}",
            reply_markup=get_education_type_keyboard(context)
        )
        return
    ...
    await update.message.reply_text(
        f"📍 {utils.get_text('choose_edu_lang', context)}",
...
async def handle_education_lang_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 🔙 Orqaga tugmasi
    if lang_text == utils.get_text('btn_back', context):
        context.user_data['state'] = 'choosing_education_type'
        return await update.message.reply_text(
    ...
    lang_code = utils.get_education_lang_code(lang_text, context)

    if not lang_code:
        await update.message.reply_text(
            f"⚠️ {utils.get_text('choose_edu_lang', context)}",
            reply_markup=get_education_lang_keyboard(context)
        )
        return
...
async def handle_course_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if course_text == utils.get_text('btn_back', context):
        if faculty == 'magistratura':
...
async def handle_complaint_type_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 🔙 Orqaga tugmasi
    if complaint_type_text == utils.get_text('btn_back', context):
        context.user_data['state'] = 'choosing_course'
        return await update.message.reply_text(
            utils.get_text('choose_course', context),
            reply_markup=get_courses_keyboard(context)
    ...
    complaint_type_code = utils.get_complaint_type_code(complaint_type_text, context)

    if not complaint_type_code:
        await update.message.reply_text(
             f"⚠️ {utils.get_text('choose_complaint_type', context)}",
             reply_markup=get_complaint_types_keyboard(context)
        )
        return
...
async def handle_subject_entry(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"📚 <b>{utils.get_text('enter_subject', context)}</b>\n└ {subject_name}\n\n"
        f"👨‍🏫 {utils.get_text('enter_teacher', context)}",
        reply_markup=get_back_keyboard(context),
        parse_mode='HTML'
    )
...
async def handle_teacher_entry(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 🔙 Orqaga tugmasi
    if teacher_name == utils.get_text('btn_back', context):
        context.user_data['state'] = 'entering_subject'
        return await update.message.reply_text(
            utils.get_text('enter_subject', context),
            reply_markup=get_back_keyboard(context)
...
async def handle_complaint_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Tasdiqlash xabari (Faqat muvaffaqiyat xabari)
    confirmation_text = utils.get_text('complaint_accepted', context)
